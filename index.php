...
<body>
    ...
    <div class="contentContainer">
        ...
        // Pagination
        $total_results_sql = "SELECT COUNT(*) as total FROM dresses";
        $result_count = mysqli_query($db, $total_results_sql);
        $row_count_result = mysqli_fetch_assoc($result_count);
        $total_results = $row_count_result['total'];

        $no_of_records_per_page = ($dresses_count > $total_results) ? 1000000 : $dresses_count;
        ...
        // Sort parameter
        $sort_param = $_GET['sort'] ?? 'id';

        // === FILTER LOGIC: buttons, multi-select, or search ===
        if (!empty($_GET['tags']) && is_array($_GET['tags'])) {
            // Tags from buttons or multi-select
    ...
    ";
        ...
        <span class="pageLinksContainer">
            <span class="dressSorting">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; margin-bottom: 20px;">
                    ...
                    <form action="index.php" method="get" style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center;">
                        ...
                        <button class="sortLink" type="submit" name="tags[]" value="Sheroes">Sheroes</button>
                        <button class="sortLink" type="submit" name="tags[]" value="Heroes">Heroes</button>
                        <button class="sortLink" type="submit" name="tags[]" value="Sarees">Sarees</button>
            ...
            <span class="pageNavConatiner">
                <tr class="pageNav">
                    <a class="pageLink pageFirst pageButton" href="?page=1&sort=<?php echo $Sort_string; ?>">
                        << First</a>
                            <td class="<?php if ($page <= 1) {
                                            echo 'disabled';
                                        } ?>">
                                <a class="pageLink pageMid pageButton" href="<?php if ($page <= 1) {
                                                                                    echo '#';
                                                                                } else {
                                                                                    echo "?page=" . ($page - 1) . "&sort=" . $Sort_string;
                                                                                } ?>">Prev</a>
                            </td>
                            <td class="<?php if ($page >= $total_pages_sql) {
                                            echo 'disabled';
                                        } ?>">
                                <a class="pageLink pageMid pageButton" href="<?php if ($page >= $total_pages) {
                                                                                    echo '#';
                                                                                } else {
                                                                                    echo "?page=" . ($page + 1) . "&sort=" . $Sort_string;
                                                                                } ?>">Next</a>
